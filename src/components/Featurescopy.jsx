...
const Features1 = () => {
  ...
  const handleOpen = (index, text1, title) => {
    ...
    if (parentDivRef.current) {
      parentDivRef.current.scrollIntoView({ behavior: "smooth" }); // Scroll to the parent div
    }
  };
  ...
  const cardVariants = {
    ...
    animate: {
      x: activeIndex === id ? 0 : 200,
      y: 0,
      opacity: activeIndex === id ? 1 : 0,
    },
  };
  ...
  return (
    <div
      id="services"
      ref={parentDivRef}
      className="pb-24 hidden lg:flex flex-col justify-center items-center overflow-hidden"
      data-aos="fade-up"
    >
      <h1 className={mulish.className + " text-5xl font-bold"}>SERVICES</h1>
      <div className="grid lg:grid-cols-4 gap-5 pt-12">
        {data.map((post, itemIndex) => (
          <motion.div
            key={post.id}
            className={` flex-col hover:bg-gradient-to-br from-[#6a913a] via-[#8dc14e] to-[#abea5d] shadow-lg shadow-black/40 border-[1px] border-black/50 hover:border-[#B9FF66] rounded-xl w-full lg:w-[20rem] aspect-square justify-between group overflow-hidden transition-colors duration-300 p-5 relative ${
              id === null ? "flex" : "hidden"
            }
              `}
            data-aos="zoom-in"
            ...
            onClick={() => handleOpen(itemIndex, post.text, post.title)}
            initial="initial"
            animate="animate"
            variants={activeIndex === id ? cardVariants : card1Variants}
            layoutId={post.id}
            style={{
              backgroundImage: isHovered[itemIndex]
                ? "linear-gradient(to bottom right, #6a913a, #8dc14e, #abea5d)"
                : `url(${post.image.src})`,
          ...
          >
            <div
              className={`w-full lg:w-[20rem] aspect-square absolute ${
                isHovered[itemIndex]
                  ? "group-hover:hidden"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="relative w-full h-full z-50 flex flex-col justify-center items-center gap-5"
          >
            <div className="flex flex-row justify-center items-center px-20">
              <div className="w-1/3">
                <motion.div
                  // key={post.id}
                  className={` flex flex-col shadow-lg shadow-black/40 border-[1px] border-black/50 hover:border-[#B9FF66] rounded-xl w-full lg:w-[20rem] aspect-square justify-between group overflow-hidden transition-colors duration-300 p-5
              `}
                  data-aos="zoom-in"
                  onMouseOver={() => handleMouseOver(itemIndex)}
                  onMouseOut={() => handleMouseOut(itemIndex)}
                  ...
                  // variants={activeIndex === id ? cardVariants : card1Variants}
                  style={{
                    backgroundImage: `url(${data[id].image.src})`,
                    backgroundSize: "cover",
                    backgroundPosition: "center",
                  }}
                >
                  <div
                    className={`w-full lg:w-[20rem] aspect-square absolute block z-20 top-0 left-0 bg-black/40 blur-sm"}`}
                  />
                  ...
                  <p
                    className={
                      mulish.className +
                      ` py-3 min-h-24 text-white transform  transition-all duration-[0.8s] z-50`
                    }
                  >
                    {data[id].description}
                  </p>
              ...
              <div className="w-2/3">
                <div className="py-5 flex justify-center items-center font-semibold lg:text-2xl">
                  {data[id].title}
                </div>
                <div
                  className="relative px-5 pb-5"
                  dangerouslySetInnerHTML={{ __html: data[id].text }}
                ></div>
              </div>
            </div>
                ...
                >
                  ...
                  {post.text !== "" && (
                    <div className="pt-[16px] border-t-2 flex justify-end z-40">
                      <button
                        onClick={() =>
                          handleOpen(itemIndex, post.text, post.title)
                        }
                        className={
                          mulish.className +
                          " shadow-black/50 shadow-lg bg-[#B9FF66] group-hover:shadow-black/60 group-hover:bg-[#363537] text-lg px-5 py-1 rounded-full text-black  group-hover:text-white group-hover:translate-x-1 group-hover:-translate-y-1 transition-all duration-300"
